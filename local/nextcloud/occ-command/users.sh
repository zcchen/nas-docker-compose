...
for k in "Public" "Downloads" "Movies" "Pictures" "Softwares"; do
    ...
    if [ ! -d "${public_homedir}/files/${k}" ]; then
        ...
        curl -u ${public_username}:${public_password} -i -L -H "OCS-APIRequest: true" -X POST \
            http://nextcloud-nginx/ocs/v1.php/apps/files_sharing/api/v1/shares \
            -F "shareType=1" -F "path=/${k}" -F "shareWith=${users_groupname}"
    fi
done
